<template>
    ...
    <div class="relative sm:pb-12 sm:ml-[calc(2rem+1px)] md:ml-[calc(3.5rem+1px)] lg:ml-[max(calc(14.5rem+1px),calc(100%-48rem))]">
      ...
      <div class="space-y-16">
        <article v-for="entry in pageStatus.visiblePosts" :key="entry.id" class="relative group">
          <div
            class="absolute -inset-y-2.5 -inset-x-4 md:-inset-y-4 md:-inset-x-6 sm:rounded-2xl bg-slate-50 group-hover:bg-slate-50/10 dark:group-hover:bg-slate-800/50"
          ></div>
          <svg
            viewBox="0 0 9 9"
            ...
            <div class="relative flex-none sm:self-start lg:self-auto xl:self-start">
              ...
              >
                <img
                  ...
                  style="width: 229px; height: 150px; left: -38px; top: 0px; opacity: 1; z-index: 2; transform-origin: 50% 50% 0px"
                />
              </div>
            </div>
            <div class="self-start flex-auto flex flex-wrap items-baseline p-5 pt-0">
              ...
              <a class="flex items-center text-sm text-sky-500 font-medium" href="/blog/2022-06-23-tailwind-templates-and-all-access"
  ...
  <div class="max-w-5xl px-4 mx-auto sm:px-6 md:px-8">
    <div class="container px-4 mx-auto mt-8">
      ...
      <ol class="relative list-none border-l border-gray-200 dark:border-gray-700">
        <li v-for="entry in pageStatus.visiblePosts" :key="entry.id" class="mb-10 ml-4">
          <div
          ...
          ></div>
          <time class="mb-1 text-xs font-normal leading-none text-gray-400 dark:text-gray-500">{{ entry.date }}</time>
          ...
          <router-link v-if="!section" :to="`/${entry.section}`" class="text-reset">
            <button class="hidden px-3 py-1 text-xs font-semibold border rounded-2xl bg-neutral-100 sm:inline-block">
              {{ entry.section }}
            </button>
          </router-link>
        </li>
      ...
      <!-- PAGINATION -->
      <ul v-if="pageStatus.endPage > pageStatus.startPage" class="flex pl-0 my-2 list-none rounded pagination" style="cursor: pointer">
        <li
